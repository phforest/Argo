...
def config(context, debug = True, rtc = False, debug_info = True, release = False, static = False):
    ...
    with context.cd(str(cfg.build_dir)):
        ...
        context.run('cmake {dir} {options}'.format(dir = THIS_DIR, options = ' '.join(options)))
...
def publish(context, verbose = False):
    ...
    def add_license():
        with open(fn_argo, 'w') as fout:
            fn_license = pathlib.Path(THIS_DIR, 'license.txt')
    ...
    def process(fn):
        ...
        with open(fn, 'r') as fin:
            ...
            for line in lines_in:
                ...
                if matches:
                    fn_sub = pathlib.Path(LIBRARY_DIR, 'inc', matches.group(1))
    ...
    add_license()
    fn_sources = [fn for fn in pathlib.Path(LIBRARY_DIR, 'src', 'argo').glob('**/*.cpp')]
    add(fn_sources)
